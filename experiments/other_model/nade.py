...
# Copy from https://github.com/tensorflow/magenta/blob/01b59d352dd2928fb94630b751d2c8e893c8701c/magenta/common/nade.py
...
class Nade(object):
    ...
    def __init__(self, num_dims, num_hidden, internal_bias=False, name='nade'):
        ...
        with tf.variable_scope(name):
            ...
            self.w_enc = tf.get_variable('w_enc', shape=[self._num_dims, 1, self._num_hidden], initializer=initializer)
            # Transposed decoder weights (`W'` in [1]).
            self.w_dec_t = tf.get_variable('w_dec_t', shape=[self._num_dims, self._num_hidden, 1], initializer=initializer)
            # Internal encoder bias term (`b` in [1]). Will be used if external biases
            # are not provided.
            if internal_bias:
                self.b_enc = tf.get_variable('b_enc', shape=[1, self._num_hidden], initializer=initializer)
            else:
                self.b_enc = None
            # Internal decoder bias term (`c` in [1]). Will be used if external biases
            # are not provided.
            if internal_bias:
                self.b_dec = tf.get_variable('b_dec', shape=[1, self._num_dims], initializer=initializer)
            else:
                self.b_dec = None
...
def teste():
    from experiments.other_model.nade import Nade
    from experiments.data.load_data_util import load_data_one_hot_encoding

    model = Nade(num_dims=6 * 117, num_hidden=500, internal_bias=True)
    data = load_data_one_hot_encoding('experiments/data').to_numpy()

    log_prob, cond_probs = model.log_prob(data)
